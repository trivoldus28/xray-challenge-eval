...
class Reject(BatchFilter):
    ...
    def __init__(
        ...
        self.logical_and_mask = logical_and_mask
        self.logical_and_mask_val = logical_and_mask_val
    ...
    def provide(self, request):
        ...
        while not have_good_batch:
            ...
            if self.mask:
                ...
                if self.logical_and_mask:
                    mask_data = np.copy(mask_data)
                    logical_array = batch.arrays[self.logical_and_mask].data
                    mask_data[logical_array != self.logical_and_mask_val] = 0
                mask_ratio = mask_data.mean()
            else:
                mask_ratio = None
