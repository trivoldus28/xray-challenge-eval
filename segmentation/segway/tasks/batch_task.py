...
class BatchTask():
    ...
    def _write_config(self, worker_filename, extra_config=None):
        ...
        self.new_worker_cmd = 'python %s run_worker %s' % (
    ...
    def _prepare_task(
        ...
        # get db host name from env if not set
        if self.db_host is None and "DAISY_DB_HOST" in os.environ:
            self.db_host = os.environ["DAISY_DB_HOST"]

        assert self.db_host is not None, "ERROR: DAISY_DB_HOST need to be set!"
    ...
    def init_callback_fn(self, context):
        ...
        print("Terminal command: DAISY_CONTEXT={} {}\n".format(
                context.to_env(), self.new_worker_cmd))
